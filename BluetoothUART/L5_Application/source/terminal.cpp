...
bool terminalTask::taskEntry()
{
    ...
    //Custom Handlers
    cp.addHandler(orientation, "orientation", "Switch the acceleration sensor task on/off");
    cp.addHandler(susp, "suspend", "Suspend selected task");
    cp.addHandler(res, "resume", "Resume selected task");
